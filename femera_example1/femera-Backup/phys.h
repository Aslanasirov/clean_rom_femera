...
public:
  // ====================================== ASLAN
  //   // virtual ~Phys() = default;
       virtual ~Phys()
           {
//                   std::cout << "Physics destructor called." << std::endl;
                       }
  //                       // ====================================== ASLAN
  ...
  // Xiaoyu: compute Pijkl
  virtual int PijklCalculation( std::ostream&, Elem*, FLOAT_SOLV*, int )=0;
  //
...
public: ElastIso2D(FLOAT_PHYS young, FLOAT_PHYS poiss, FLOAT_PHYS thick) :
  ...
  // Xiaoyu
  int PijklCalculation(std::ostream&, Elem*, FLOAT_SOLV*, int) final;
...
public: ElastIso3D(FLOAT_PHYS young, FLOAT_PHYS poiss ) :
  ...
  // Xiaoyu: add new constructor
  ElastIso3D(FLOAT_PHYS young, FLOAT_PHYS poiss, int dGrain ) :
  Phys((Phys::vals){young,poiss}){
    int nRHS = 6;
    int Dm = 3;
    this->node_d = dGrain*nRHS*Dm;
    //this->elem_flop = 225;//FIXME Tensor eval for linear tet
    // calc stiff_flop from (node_d*E->elem_node_n)*(node_d*E->elem_node_n-1.0)
    ElastIso3D::MtrlProp2MatC();
  };
...
public:
  ...
  ElastOrtho3D(// Orthotropic Material Constructor
    ...
    Phys( prop,dirs )
     { node_d = 3; ElastOrtho3D::MtrlProp2MatC(); };
  ElastOrtho3D(// Orthotropic Material Constructor for solving multiple PhUCPs at one time
    Phys::vals prop, Phys::vals dirs, int dGrain ) :
    Phys( prop,dirs )
     { int nRHS = 6; int Dm = 3; this->node_d = dGrain*nRHS*Dm; ElastOrtho3D::MtrlProp2MatC(); };
#if 0
  int SavePartFMR( const char* bname, bool is_bin ) final;
  int ReadPartFMR( const char* bname, bool is_bin ) final;
#endif
