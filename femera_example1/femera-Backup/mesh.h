...
//                   std::cout << "Mesh destructor called." << std::endl;
  ...
  int ReadPartFMR( part&, const char* bname, bool is_bin, int );
  // Xiaoyu: add input for dGrain
  int Setup(int);
  int Setup();//FIXME Not yet used?
  virtual int Init()=0;
  // Xiaoyu: add init(StartGrain, EndGrain)
  virtual int Init(int StartGrain, int EndGrain)=0;
  virtual int Iter()=0;
  int GatherGlobalIDs();
  int ScatterHaloIDs();
  int SyncIDs();
...
public:
  ...
  // Xiaoyu: add init(iGrain)
  int Init(int StartGrain, int EndGrain) final;
  int Init() final;
  int Iter() final;
protected:
private:
