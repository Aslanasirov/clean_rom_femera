...
int ElastIso3D::ElemLinear( Elem* E, const INT_MESH e0, const INT_MESH ee,
  FLOAT_SOLV* part_f, const FLOAT_SOLV* part_u ){
	// printf("Get in elas-iso3-base.cc \n");
	...
	const int Ne = Dn*Nc;// Number of DOFs / element
	...
	for (int iGrain = 0; iGrain < dGrain; iGrain++){
		...
		//	printf("iRHS is %i \n", iRHS);
		//	printf("before if, u was \n");
		//	for (int i=0; i<12; i++){ printf("%16.14e \n",u[i]);}
		//	printf("e0 is %i \n", e0);
		//	printf("part_u[0:12] was \n");
		//	for (int i=0; i<12; i++){ printf("%16.14e \n",part_u[i]);}
		//	printf("S_u[0:12] was \n");
		//	for (int i=0; i<12; i++){ printf("%16.14e \n",S_u[i]);}
			if(e0<ee){
		...
		//				printf("E_c[Nc*e0+i] is %i \n", E_c[Nc*e0+i]);
		//				printf("E_c[Nc*e0+i]*Dnt+iRHS*Dn is %i \n", E_c[Nc*e0+i]*Dnt+iRHS*Dn);
		//				printf("S_u[E_c[Nc*e0+i]*Dnt+iRHS*Dn] is %16.14e \n", S_u[E_c[Nc*e0+i]*Dnt+iRHS*Dn]);
						std::memcpy( & u[Dn*i],& S_u[E_c[Nc*e0+i]*Dnt*dGrain+iGrain*nRHS*Dn+iRHS*Dn], sizeof(FLOAT_SOLV)*Dn ); }
						//std::memcpy( & u[Dn*i],& S_u[E_c[Nc*e0+i]*Dn], sizeof(FLOAT_SOLV)*Dn ); }
				#endif
			}
		//	printf("after if, u was \n");
		//	for (int i=0; i<12; i++){ printf("%16.14e \n",u[i]);}
		...
		//		for (int i=0; i<12; i++){ printf("%16.14e \n",u[i]);}
				...
				for (int i=0; i<Nc; i++){
					...
					std::memcpy( & f[Dn*i],& S_f[E_c[Nc*ie+i]*Dnt*dGrain+iGrain*nRHS*Dn+iRHS*Dn], sizeof(FLOAT_SOLV)*Dn ); }
					...
					if(ip==(intp_n-1)){
						if((ie+1)<ee){// Fetch stuff for the next iteration
							...
							for (int i=0; i<Nc; i++){
								...
								// Xiaoyu
								std::memcpy(&u[Dn*i],&S_u[c[i]*Dnt*dGrain+iGrain*nRHS*Dn+iRHS*Dn], sizeof(FLOAT_SOLV)*Dn);
		//						printf("While fetching stuff for the next iteration, \n");
		//						printf("c[i] is %i \n",c[i]);
		//						printf("c[i]*Dnt+iRHS*Dn is %i \n",c[i]*Dnt+iRHS*Dn);
		//						printf("S_u is %16.14e, %16.14e, %16.14e \n", S_u[c[i]*Dnt+iRHS*Dn], S_u[c[i]*Dnt+iRHS*Dn+1], S_u[c[i]*Dnt+iRHS*Dn+2]);
		...
		//		for (int i=0; i<12; i++){ printf("%16.14e \n",f[i]);}
				for (uint i=0; i<uint(Nc); i++){
					// Xiaoyu
					std::memcpy(& S_f[E_c[Nc*ie+i]*Dnt*dGrain+iGrain*nRHS*Dn+iRHS*Dn],& f[Dn*i], sizeof(FLOAT_SOLV)*Dn );
