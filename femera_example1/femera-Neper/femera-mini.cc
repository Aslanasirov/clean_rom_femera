...
int main( int argc, char** argv ){
  ...
  int step_n     = 1;
  int iter_max   =-1;
  // Xiaoyu: define number of UCP solved at one time
  int dGrain  = 1;
  int GrainID1 = 1;// Starting ID
  int GrainID2 = 1;// Ending ID. Solve PhUCP from GrainID1 to GrainID2 in the current code
  ...
  opterr = 0; int c;
  while ((c = getopt (argc, argv, "v:pP:h:c:g:j:k:m:n:s:I:i:r:a:o:x:y:z:V:d:u:"))
    != -1){
    // x:  -x requires an argument
    switch (c) {
      case 'v':{ verbosity= atoi(optarg); break; }
      case 'V':{ simd_n   = atoi(optarg); break; }
      // Xiaoyu: add option for dGrain, GrainID1 and GrainID2
      case 'g':{ dGrain   = atoi(optarg); break; }
      case 'j':{ GrainID1 = atoi(optarg); break; }
      case 'k':{ GrainID2 = atoi(optarg); break; }
#ifdef _OPENMP
#if OMP_NESTED==true
...
#endif
// Xiaoyu: add for loop over parts (1 part == 1 grain in this study)
printf("GrainID1 is %i \n",GrainID1);
printf("GrainID2 is %i \n",GrainID2);
for (int iGrain=GrainID1-1; iGrain<GrainID2; iGrain=iGrain+dGrain){
	  // Read and Setup =============================================
	  int iter=0;
	#if VERB_MAX>0
	...
	#endif
	printf("This is grain %i \n",iGrain);
	  M->Setup(dGrain);
	#if VERB_MAX>1
	  //if(verbosity>1){ fflush(stdout); }
	#endif
	  {// scope local variables
	  printf("======================== 338");//int sugg_max=3000;
	  iter_info_n =   1; printf("======================== 339"); // sugg_max = M->udof_n;
	  ...
	  }else                          { iter_info_n =   1;printf("======================== 343");// sugg_max =M->udof_n; 
	  }
	  if(iter_max<0){
	    iter_max = M->udof_n / iter_info_n *10; printf("======================== 346");
	    if( long(iter_max) > long(M->udof_n) ){ iter_max=int(M->udof_n);printf("======================== 347"); }
	#if 0
	...
	printf("======================== 353"); //endif
	#if VERB_MAX>0
	  setu_done = std::chrono::high_resolution_clock::now();printf("======================== 355");
	  read_time = std::chrono::duration_cast<std::chrono::nanoseconds>
	    (setu_done-read_start);printf("======================== 357");
	  read_sec=float(read_time.count())*1e-9;printf("======================== 358");
	  //if(verbosity==1){
	 ...
	 // }
	  ...
	  {// load step init scope
	    ...
	    // Xiaoyu: 
	    M->Init(iGrain,iGrain+dGrain-1);//printf("**** Hello Mini! ****\n");
	...
	//for(int i=part_0; i<part_n+part_0; i++){
	//			Elem* E; Phys* Y; Solv* S; 
	//			std::tie(E,Y,S)= M->priv_part[i];
	//			delete E; delete Y; delete S;
	//		}
	//delete M;
	//=============================================================================================================
	...
	#pragma omp critical(minmax)
	{
		...
		// Xiaoyu
		// /*
		printf("%i,%i,%i\n",iGrain,part_i,dGrain);
		// --------------------- Compute Mijkl ----------------------------
		int nRHS = 6;
		FLOAT_PHYS Mijkl[6*6];
		FLOAT_PHYS invM[6*6];
		//int Switch[6] = {0,1,2,3,5,4};//{1,2,3,6,5,4}; Isotropic
		...
		FLOAT_PHYS R[]={
			Y->mtrl_rotc[0],Y->mtrl_rotc[1],Y->mtrl_rotc[2],
			Y->mtrl_rotc[3],Y->mtrl_rotc[4],Y->mtrl_rotc[5],
			Y->mtrl_rotc[6],Y->mtrl_rotc[7],Y->mtrl_rotc[8]};
		FLOAT_PHYS R6x6[36];
		int J1, J2, I1, I2;
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 6; j++){
				M[6*i+j] = 0.0;
				for (int k = 0; k < 6; k++) {
					M[6*i+j] += RM[6*i+k]*R6x6[6*j+k];
				}
			}
		}
	    	// Compute the inverse of M
		double L[36], U[36], b[6], darray[6], x[6];
		for(int i=0;i<36;i++){ L[i]=0.0; };
		for(int i=0;i<36;i++){ U[i]=0.0; };
		for(int i=0;i<6;i++){ b[i]=0.0; };
		for(int i=0;i<6;i++){ darray[i]=0.0; };
		for(int i=0;i<6;i++){ x[i]=0.0; };
		for (int k = 0; k<5; k++){
			for (int i = k+1; i<6; i++){
				double coeff=M[i*6+k] / M[k*6+k];
				L[i*6+k] = coeff;
				for (int j = k+1; j<6; j++){
					M[i*6+j] = M[i*6+j] - coeff*M[k*6+j];
				}
			}
		}
		for (int i = 0; i<6; i++){
		...
		for (int k = 0; k<6; k++){
			...
			for (int i = 1; i<6; i++){
				darray[i] = b[i];
				for (int j = 0; j<=i-1; j++){
					darray[i] = darray[i] - L[i*6+j]*darray[j];
				}
			}
			x[5] = darray[5] / U[5*6+5];
			for (int i = 4; i>=0; i--){
				x[i] = darray[i];
				for (int j = 5; j>=i+1; j--){
					x[i] = x[i] - U[i*6+j]*x[j];
		...
		// Compute Mijkl
		for(int i=0;i<nRHS*nRHS;i++){ Mijkl[i]=invM[i]; };
		for (int irow=0; irow<nRHS; irow++){
			for (int icol=0; icol<nRHS; icol++){
				Mijkl[irow*nRHS+icol] = invM[Switch[irow]*nRHS+Switch[icol]];
			}
		}
		for (int irow=0; irow<nRHS; irow++){
			printf("%15.13f, %15.13f, %15.13f, %15.13f, %15.13f, %15.13f\n", Mijkl[irow*nRHS+0], Mijkl[irow*nRHS+1], Mijkl[irow*nRHS+2], Mijkl[irow*nRHS+3], Mijkl[irow*nRHS+4], Mijkl[irow*nRHS+5]);
		}
		// Compute Pijkl
		Y->PijklCalculation( std::cout, E, S->part_u, part_n );
		// xiaoyu: end of PijklCalculation */
	}
	//delete E;
	//delete Y;
	//delete S;
	...
	#endif
	// xiaoyu: comment out following code
	/*
	printf("SF after ElemStrainStress!");
	#ifdef HAS_TEST
	    // Check solution ===============================================
	#if VERB_MAX>1
	    if( verbosity>1 ){
	...
	#if VERB_MAX > 2
	      if(verbosity>2){
		// Xiaoyu: comment out the following line
		if(S->udof_n<300){
	...
	#endif //HAS_TEST
	*/
	// Xiaoyu: end of commenting out the entire section after results printing
	  }//load step loop
	  }//load step scope
	//M->~Mesh();
	...
	{	
			// if(iGrain>0){
			// 			// Elem* E; Phys* Y; Solv* S; 
			// 						// std::tie(E,Y,S)=M->mesh_part[iGrain-1];
			// 									// delete E; E=NULL;
			// 												// delete S; S=NULL;}
			//
			// 															// for(int part_i=part_0; part_i < (part_0+part_n); part_i++){
	for(int part_i=part_0; part_i < (part_0+part_n); part_i++){
	Elem* E; Phys* Y; Solv* S; 
	std::tie(E,Y,S)=M->mesh_part[part_i];
	delete Y; Y=NULL;
	delete E; E=NULL;
	delete S; S=NULL;
	}
  ...
  // Xiaoyu: end of for loop over parts
#if OMP_NESTED==true
