...
// Xiaoyu: keep the original Setup()
int Mesh::Setup(){
...
// Xiaoyu: add new method
int Mesh::Setup(int dGrain){
  int part_0=1; bool is_part=true;//FIXME
  int part_n=int(this->mesh_part.size()) - part_0;
  //
  int dots_mod=0;
...
#endif
    Mesh::part t;
    this->ReadPartFMR(t,pname.c_str(),false,dGrain);
    Elem* E; Phys* Y; Solv* S; std::tie(E,Y,S)=t;
    Y->solv_cond = this->solv_cond;
    S->solv_cond = this->solv_cond;
    if(S->solv_cond==Solv::COND_JAC3){ S->cond_bloc_n=3; };
...
#endif
  printf("End of Mesh->Setup \n");
  return 0;
}
// Xiaoyu: keep the original ReadPartFMR
...
// Xiaoyu: add dGrain as input
int Mesh::ReadPartFMR( part& P, const char* fname, bool is_bin, int dGrain ){
  Elem* E; Phys* Y; Solv* S; std::tie(E,Y,S)=P;
  INT_ORDER pord=1;
  E = new Tet(pord);//FIXME
  E->simd_n = this->simd_n;//FIXME
...
//    }
    ...
    if(t_elas_prop.size()<21){
      if(t_mtrl_dirs.size()<3){
        // Xiaoyu: add dGrain as input
        Y = new ElastIso3D(t_elas_prop[0],t_elas_prop[1],dGrain);
        //Y = new ElastIso3D(t_elas_prop[0],t_elas_prop[1]);
#if VERB_MAX>1
#pragma omp atomic update
        this->iso3_part_n+=1;
        ...
        // Xiaoyu: add dGrain as input
        Y = new ElastOrtho3D(t_elas_prop,t_mtrl_dirs,dGrain);
        //Y = new ElastOrtho3D(t_elas_prop,t_mtrl_dirs);
#if VERB_MAX>1
#pragma omp atomic update
        this->ort3_part_n+=1;
